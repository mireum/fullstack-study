...
function CounterEffect(props) {
  ...
  }); // 의존성 배열이 없으면 componentDidMount(처음 마운트 됐을 때 호출), componentDidUpdate(업데이트 될 때 호출)와 같은 방식으로 동작
  // }, []); // 처음 마운트 될 때만 실행됨, componentDidMount만
  ...
  return (
    <>
      ...
      <button onClick={() => {
        ...
        document.title = `You clicked ${count} times`;
        // 차이점
        // 1. 초기에 타이틀이 안 바뀌어 있음(버튼을 누르지 않은 상태, 처음 마운트 시에 실행 안됨)
        // 2. 버튼 클릭 시 count 값 반영이 제대로 안됨(set함수는 비동기함수, 넘어감)

        // onClick과 useEffect는 실행 시점이 다르기에 동작이 다를 수 밖에 없음(완전히 다른 용도)
...
// => 시간이 걸리는 작업에 해당, useEffect를 안쓰고 동기식으로 요청을 보내고
// 서버에서 응답이 없으면 화면 렌더링이 안됨
