...
function ReservationRefactoring() {
  ...
  // 물론, 관련이 없는 데이터라면 여러 개의 state로 선언해서 사용한다
  ...
  const handleInputChange = (e) => {
    ...
    // 중요!!
    // 리액트 상태에서 객체를 수정해야할 때에는,
    // 아래와 같이 기존 상태를 직접 수정해서 set함수에 넣으면 안된다
    // (inputs가 가리키는 객체의 내부 데이터만 바뀐 것이지 주소값(참조값)은 변하지 않았기 때문)
    // inputs[name] = inputValue;
    // setInputs(inputs); // set함수를 써도 재렌더링 발생 안함(리액트가 객체가 바뀐 것을 감지하지 못함)

    // 그 대신 새로운 객체를 만들어서 새로운 객체에 변화를 주고 이를 상태로 사용해야 한다
    // 이러한 작업을 "불변성을 지킨다" 라고 부름(기존 객체를 변경하지 않는다)
    // 불변성을 지켜주어야만 리액트 컴포넌트에서 상태가 업데이트가 됐음을 감지할 수 있고 이에 따라 재렌더링이 된다
    // 방법1
    // const copyInputs = {
    //   ...inputs
    // };
    ...
    // 결론: 리액트에서 객체를 업데이트할 때에는 기존 객체를 직접  수정하면 안되고
  ...
  // 여러 input 앨리먼트를 제어해야할 때, 각 엘리먼트에 name 속성을 추가하고 
