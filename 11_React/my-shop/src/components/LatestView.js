...
function LatestView(props) {
  ...
  const productList = useSelector(selectProductList); // 전역 스토어에서 꺼내옴, 처음 새로고침 시 초기값 빈 배열
    ...
    <LatestViewWrapper>
      ...
      <ListGroup variant="flush">
        {latestViewedProducts.slice(0, 3).map((product) => {
          // 주의: key 속성은 가장 최상위 엘리먼트에 부여, <></>에는 속성 사용 불가, 풀네임 써줘야함
      ...
      {/* 3개까지만 표출하고 초과 시 최근 본 상품 페이지로 이동 */}
